...
const Club = () => {
  ...
  const [courseLevel, setLevel] = useState('member');
  ...
  useEffect(() => {
    if (gpaStatus === true) {
      Swal.fire({
        title: `Club Details has been successfully saved`,
      ...
      Swal.fire({
        title: `Error While Saving Club details`,
        // text: "You won't be able to revert this!",
        icon: 'error',
        showCancelButton: false,
        confirmButtonColor: '#3085d6',
  ...
  const submitGPA = () => {
    if (!name || fileVal.length === 0) {
      ...
      formData.append('club_level', courseLevel);
